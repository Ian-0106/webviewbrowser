...
static int MeasureTabWidth(const std::wstring &text) {
...
static void ComputeTabLayout() {
    ...
    for (size_t i = 0; i < g_tabs.size(); i++) {
        std::wstring label =
            g_tabs[i].title.empty() ? L"New Tab" : g_tabs[i].title;
        int tw = MeasureTabWidth(label);
...
static void DrawTextCentered(HDC dc, const RECT &rc, const std::wstring &text,
                             HFONT font, COLORREF color, UINT extraFlags = 0) {
    HFONT old = (HFONT)SelectObject(dc, font);
    SetTextColor(dc, color);
    SetBkMode(dc, TRANSPARENT);
    RECT trc = rc;
    DrawTextW(dc, text.c_str(), (int)text.size(), &trc,
...
static void PaintTabStrip(HDC dc) {
    ...
    for (size_t i = 0; i < g_tabGeom.size() && i < g_tabs.size(); i++) {
        ...
        if (closeHover || closePress) {
            HBRUSH cb = CreateSolidBrush(closePress ? kHoverTab : kCloseHover);
            HPEN cp = CreatePen(PS_SOLID, 1,
                                closePress ? kHoverTab : kCloseHover);
            HGDIOBJ ob = SelectObject(mem, cb);
            HGDIOBJ op = SelectObject(mem, cp);
            Ellipse(mem, cx, cy, cx + d, cy + d);
            SelectObject(mem, ob);
            SelectObject(mem, op);
            DeleteObject(cb);
            DeleteObject(cp);
    ...
    if (plusHover || plusPress) {
        HBRUSH pb = CreateSolidBrush(plusPress ? kPressTab : kHoverTab);
        HPEN pp = CreatePen(PS_SOLID, 1, plusPress ? kPressTab : kHoverTab);
        HGDIOBJ ob = SelectObject(mem, pb);
        HGDIOBJ op = SelectObject(mem, pp);
        Ellipse(mem, px, py, px + pd, py + pd);
        SelectObject(mem, ob);
        SelectObject(mem, op);
        DeleteObject(pb);
        DeleteObject(pp);
